...
interface ProductDao {
    ...
    @Query("""
        SELECT p.id as productId, p.name as productName, COALESCE(SUM(b.price * ref.amountUsed), 0.0) as totalCost
        FROM products p
    ...
    @Transaction
    suspend fun insertProductWithBahanBaku(product: Product, pairs: List<Pair<Int, Double>>) {
        ...
        val crossRefs = pairs.map { ProductBahanBakuCrossRef(actualProductId, it.first, it.second) }
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE bahan_baku ADD COLUMN minAmount REAL NOT NULL DEFAULT 0.0")
            }
        }
...
class ProductRepository(private val productDao: ProductDao, private val bahanBakuDao: BahanBakuDao) {
    ...
    suspend fun insert(product: Product) {
        val pairs = product.bahanBakuIds.split(",")
            .map { it.trim() }
            .filter { it.isNotEmpty() }
            .map { it.split(":") }
            .map {
                val id = it[0].trim().toIntOrNull() ?: 0
                val amount = if (it.size > 1) it[1].toDoubleOrNull() ?: 1.0 else 1.0
                id to amount
            }
            .filter { it.first != 0 }
        productDao.insertProductWithBahanBaku(product, pairs)
    }
